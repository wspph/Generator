...
import org.wspph.Exception.BindException;
import org.wspph.domain.Column;
import org.wspph.domain.Table;
import java.io.File;
...
public class GenUtils {
    ...
    public static void generatorCode(Map<String, String> table,List<Map<String, String>> columns, ZipOutputStream zip) {
        ...
        for (String template : templates) {
            ...
            try {
                //添加到zip
                zip.putNextEntry(new ZipEntry(getFileName(template , tableDO.getClassName(), config.getString("package").substring(config.getString("package").lastIndexOf(".") + 1))));
                IOUtils.write(sw.toString(), zip, "UTF-8");
                IOUtils.closeQuietly(sw);
                zip.closeEntry();
            } catch (IOException e) {
                throw new BindException("渲染模板失败，表名：" + tableDO.getTableName(), e);
            }
        }
    }
    ...
    public static Configuration getConfig() {
        try {
            ...
            throw new BindException("获取配置文件失败，", e);
        }
    }
